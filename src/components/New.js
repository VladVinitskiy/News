...
const New = ({onClick, onAddComment, onAddLike, showMore, userName, status, author, text, bigText, isAuthorize, index, more, comments, like}) => {
    ...
    return (
        ...
        >
            ...
            {more === true ? <p className='h4 p-1 font-weight-normal text_mode'>{bigText}</p> :
                <p className='h3 p-1 font-weight-normal text_mode'>{text}</p>}
            ...
            {comments.map((item, index) => {
                return <p key={index}
                          className='h3 my-2 my-lg-3 my-md-3 my-sm-2 font-italic text_mode'>{item.user}: {item.comment}</p>
            })}
        </li>
    )
};
